...
if args.case == 1:
    ...
    omegaUpper = omega0 * 1.2
    omegaLower = omega0 * 0.8
else:
...
class Top(Multipoint):
    def setup(self):
        ...
        nonlinear_solver = om.NonlinearBlockGS(maxiter=10, iprint=2, use_aitken=True, rtol=1e-6, atol=100.0)
        linear_solver = om.LinearBlockGS(maxiter=5, iprint=2, use_aitken=True, rtol=1e-5, atol=1e0)
        ...
        for discipline in ["aero"]:
            self.connect("geometry.x_%s0_geometry_output" % discipline, "hover.x_%s0_masked" % discipline)
        for discipline in ["struct"]:
            self.connect("geometry.x_%s0_geometry_output" % discipline, "hover.x_%s0" % discipline)

        # more manual connection
        self.connect("mesh_aero.x_aero0", "geometry.x_aero0_geometry_input")
        self.connect("mesh_struct.x_struct0_mesh", "geometry.x_struct0_geometry_input")

        self.add_subsystem("CT", om.ExecComp("value=T/1.178/((omega/2/3.14159)**2*(R*2)**4)"))
        self.add_subsystem("CP", om.ExecComp("value=P/1.178/((omega/2/3.14159)**3*(R*2)**5)"))
    ...
    def configure(self):
        ...
        self.hover.coupling.aero.solver.add_dvgeo(self.geometry.DVGeo)

        # add the dv_con object to the builder solver. This will be used to write deformed constraints
        self.hover.coupling.aero.solver.add_dvcon(self.geometry.DVCon)
        ...
        self.hover.coupling.aero.solver.add_dv_func("MRF", MRF)
        ...
        leRList = [[-0.003564, -0.013675, 0.035], [-0.003564, -0.013675, 0.145]]

        self.geometry.nom_addLERadiusConstraints("leradius", leRList, nSpan=10, axis=[1, 0, 0], chordDir=[0, -1, 0])
        ...
        self.add_design_var("MRF", lower=omegaLower, upper=omegaUpper, scaler=0.1)
        ...
        # objective function
        self.add_objective("CP.value", scaler=10.0)
        self.connect("hover.aero_post.power", "CP.P")
        self.connect("MRF", "CP.omega")
        self.connect("hover.aero_post.r1", "CP.R")

        # constraints
        self.add_constraint("CT.value", equals=ct_target, scaler=10.0)
        self.connect("hover.aero_post.thrust", "CT.T")
        self.connect("MRF", "CT.omega")
        self.connect("hover.aero_post.r1", "CT.R")
        ...
        self.add_constraint("hover.ks_vmfailure", lower=-100.0, upper=0.5, scaler=1.0)
        self.add_constraint("hover.mass", lower=-100.0, upper=m0 * 1.1, scaler=10.0)
        self.add_constraint("geometry.curvature1", upper=1.5, scaler=1.0)
        self.add_constraint("geometry.curvature2", upper=1.5, scaler=1.0)
        self.add_constraint("geometry.leradius", lower=0.8, upper=3.0, scaler=1.0)
...
if args.task == "opt":
    ...
    prob.run_model()
elif args.task == "runPrimalDeformed":
    # Import the design variable json file
    with open("./designVariables.json") as f:
        dvDict = json.load(f)
    # assign the loaded design var to the OM prob
    for dvName in dvDict:
        prob.set_val(dvName, dvDict[dvName])
    # just run the primal once
    prob.run_model()
