...
class Top(Multipoint):
    ...
    def configure(self):
        ...
        # Set up the sweep variable, here val[0] is the sweep change degree
        def sweep(val, geo):
            refAxisCoef = geo.extractCoef("wingAxis")
            refAxisS = geo.refAxis.curves[0].s
            sweepDeg = np.deg2rad(float(val[0]))
            xDelta = np.tan(sweepDeg) * span0
            for i in range(len(refAxisCoef)):
                refAxisCoef[i, 0] = refAxisCoef[i, 0] + refAxisS[i] * xDelta
            geo.restoreCoef(refAxisCoef, "wingAxis")

        # add span variable
        self.geometry.nom_addGlobalDV(dvName="sweep", value=np.array([0]), func=sweep)
        ...
        self.dvs.add_output("sweep", val=np.array([0]))
        ...
        self.connect("sweep", "geometry.sweep")
        ...
        self.add_design_var("sweep", lower=0.0, upper=30.0, scaler=0.01)
